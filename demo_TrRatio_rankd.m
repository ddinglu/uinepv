...
% n = 800, 20 samples, timing estimate, 40m

n = 60; k = 40; 

% -- set number of sampled alpha to evaluate spectral radius and observed rate
nsample = 12*17; % 12 sampled * integer
thetan = flip(linspace(-.5, 1.5, nsample+1)); 	% for spectral radius plot
sample_theta = flip(linspace(-0.5, 1.5, 12+1));	% for observed rate 
...
for rk = RKS 
	...
	[sample_conv, sample_sprd, res_hist] = SolveNEPv_mod(A, B, D, D1, P, 2, sample_theta, 0, mtol);
	disp('--- ') 
	disp('Generate spectral radius:')
	[conv_hist, sprd_hist] = SolveNEPv(A, B, D, D1, P, 2, thetan, 0, mtol);
	...
	% ---------------------------------------------------------------------
    figure(1);
    h1{ind} = plot(thetan, sprd_hist, '-', 'LineWidth', 2, 'DisplayName',['$',exlabel,'=$ ',num2str(rk)], 'color', corder(ind,:)); hold on;

    figure(2); 
    h2{ind} = plot(thetan, sprd_hist, '-', 'LineWidth', 2, 'DisplayName',['$',exlabel,'=$ ',num2str(rk)], 'color', corder(ind,:)); hold on;
    h3{ind} = plot(sample_theta, sample_conv, 'o','MarkerSize',6,'LineWidth',1, 'color', corder(ind,:)); hold on;
	ind = ind+1;

end % loop over rank

figure(1); % spectral radius
plot([0,0], [0.6,1], '--k'); plot([1,1], [0.6,1], '--k');
xlabel('$\theta$');
ylabel('spectral radius');
legend([h1{1},h1{2},h1{3},h1{4}],'Location','southeast','Interpreter','latex','FontSize',12);

figure(2); % rate of convergence
plot([0,0], [0.6,1], '--k'); plot([1,1], [0.6,1], '--k');
xlabel('$\theta$');
ylabel('convergence rate');
legend([h2{1},h2{2},h2{3},h2{4}],'Location','southeast','Interpreter','latex','FontSize',12);

% ------------------------------------------------------------------------
% 3. MISC: Save computation result for large simulation
% ------------------------------------------------------------------------
if n >= 200
	save(['nepv7_2_',ex,'_n',num2str(n),'.mat']);
end
